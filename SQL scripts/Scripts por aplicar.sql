DROP VIEW if EXISTS vistacoasientoe;

CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vistacoasientoe` AS SELECT 
	no_comprob,
	fecha_comp,
	no_refer,
	tipo_comp,
...
CREATE ALGORITHM = UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vistaconsecutivoasientos` AS SELECT 
...
CREATE ALGORITHM = UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vistacocatalogo` AS SELECT 
	CONCAT(`cocatalogo`.`mayor`,`cocatalogo`.`sub_cta`,`cocatalogo`.`sub_sub`,`cocatalogo`.`colect`) AS `cuenta`,
	`cocatalogo`.`mayor` AS `mayor`,
	`cocatalogo`.`sub_cta` AS `sub_cta`,
	`cocatalogo`.`sub_sub` AS `sub_sub`,
	`cocatalogo`.`colect` AS `colect`,
	`cocatalogo`.`nom_cta` AS `nom_cta`,
	`cocatalogo`.`nivel` AS `nivel`,
	`cocatalogo`.`tipo_cta` AS `tipo_cta`,
	`cocatalogo`.`fecha_upd` AS `fecha_upd`,
	`cocatalogo`.`ano_anter` AS `ano_anter`,
	`cocatalogo`.`db_fecha` AS `db_fecha`,
	`cocatalogo`.`cr_fecha` AS `cr_fecha`,
	`cocatalogo`.`db_mes` AS `db_mes`,
	`cocatalogo`.`cr_mes` AS `cr_mes`,
	`cocatalogo`.`nivelc` AS `nivelc`,
	`cocatalogo`.`nombre` AS `nombre`,
	`cocatalogo`.`fecha_c` AS 
	`fecha_c` 
FROM `cocatalogo`  ;
...
BEGIN
	...
	*/
	...
	if vError = 0 AND EXISTS(
		SELECT h.nom_cta
			FROM hcocatalogo h
			LEFT JOIN cocatalogo c ON h.mayor = c.mayor
