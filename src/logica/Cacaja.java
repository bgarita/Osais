...
/**
 * Clase de cajas. Es igual que la tabla caja. Nota 1: Si la caja está bloqueada
 * solo permite transacciones del usuario asignado. Nota 2: Si la caja está
 * cerrada no permite transacciones de nadie.
 *
...
public class Cacaja implements IEstructuraBD {
    ...
    private final Bitacora logo = new Bitacora();
    ...
    public void setUser(String user) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sqlSent,
            ...
            logo.writeToLog(this.getClass().getName() + "--> " + ex.getMessage(), Bitacora.ERROR);
            return;
        } // end try-catch
    ...
    // </editor-fold>
    /**
     * @author Bosco Garita 20/04/2015 Este método trae todos los campos de la
    ...
    /**
     * @author Bosco Garita 20/04/2015 Este método trae todos los campos de toda
    ...
    /**
     * Este método consulta la base de datos para ver si la caja existe o no.
     *
     ...
     * fisico y fechaInicio. Nota: No controla transacciones ni verifica si el
     * registro existe. Esto es una labor que debe hacer antes el programa que
     * lo invoque.
     *
     ...
     * campo idcaja de la clase esté inicializado con el valor que se eliminará.
     ...
     * que se bloqueará. Nota: No realiza ningua verificación. Esto es una labor
     * que debe hacer antes el programa que lo invoque.
     *
     * @author Bosco Garita 21/04/2015
     * @param bloquear boolean true=bloquear, false=desbloquear
     * @return boolean true=tuvo éxito, false=falló verificar el mensaje de
     * error (getMensaje_error())
     *
     */
    ...
    /**
     * Este método solo se usa para inicializar los campos de la clase con los
     * valores predeterminados (los que pueden tener valores default).
     */
    @Override
    ...
    /**
     * Este método recalcula los campos depositos, retiros, efectivo y
     * saldoactual. Lo usa el proceso de cierre de caja, la pantalla de
     * mantenimiento de cajas y el método Catransa.anularRegistro. El cálculo se
     * basa en los movimientos registrados en la tabla catransa. Esta tabla solo
     * tiene los registros de las cajas abiertas.
     */
     ...
     * actual de la tabla catransa a la tabla hcatransa y luego las elimina de
     * la tabla catransa, además inserta el registro de la tabla caja en la
     * tabla hcaja, establece el campo saldoinicial = fisico y pone en cero los
     * depósitos y retiros recalculando nuevamente el saldo de la caja. Este
     * proceso tiene implísito el control transaccional.
     *
        ...
        try {
            ...
            if (hayTran) {
                try {
                    ...
                    logo.writeToLog(this.getClass().getName() + "--> " + ex1.getMessage(), Bitacora.ERROR);
                    System.exit(1);
                } // end try-interno
            } // end if
    ...
    /**
     * @author: Bosco Garita 23/04/2015 Este método abre la caja actual para que
     * acepte transacciones. El usuario debe ser un cajero activo y la caja no
     * debe estar bloqueada por otro cajero. Nota: se considera que la caja está
     * abierta cuando el campo bloqueada = "S".
    ...
    /**
     * @author Bosco Garita 22/05/2015 Actualiza los depósitos, retiros, saldo
     * actual y efectivo.
     */
    ...
    /**
     * Este método verifica si existen transacciones (que impliquen recepción de
     * dinero) sin aplicar en caja y de ser así escribe el mensaje
     * correspondiente para que el método de cierre de caja no continue.
     *
