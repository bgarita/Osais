...
public class Cuenta {
    ...
    public Cuenta(Connection c){
        this.conn = c;
        String sqlSent =
               "Select * from cocatalogo " +
               "Where (mayor + sub_cta + sub_sub + colect) = " +
               "(Select min(mayor + sub_cta + sub_sub + colect) from cocatalogo)";
        try {
    ...
    /**
     * Este método solo valida el campo de cuenta (mayor, sub_cta,
     * sub_sub o colect).  No afecta en nada a los valores propios de
     * la clase.
     * El campo solo puede ser válido si viene vacío o si
     * sus tres caracteres son números.
     * @param f
     * @return 
     */
    ...
    /**
     * Inicializa la cuenta descomponiendo el string recibido.
     * Como mínimo debe recibir tres caracteres numéricos, que corresponderían
     * a la cuena mayor.  El resto se rellena de ceros.
    ...
    /**
     * Este método toma los diferentes niveles que componen la cuenta
     * contable, los concatena y los devuelve como un solo string.
    ...
    /**
     * Este método carga los datos correspondientes a una cuenta contable.
     * Antes de hacerlo verifica que cada campo que compone la cueta sea válido
     * porque el registro que cargará corresponde a los valores que contengan
     * los campos mayor, sub_cta, sub_sub y colect de esta misma clase.
     * En caso de que no se cargue nada solo puede haber dos razones: la primera
     * es porque se produjo algún error en cuyo caso se debe consultar los 
     * métodos isError() y getMensaje_error().  La segunda razón es porque la
    ...
    public final void cargarRegistro(Connection c) {
        ...
        String sqlSent =
                "Select nom_cta,nivel, tipo_cta from cocatalogo " +
