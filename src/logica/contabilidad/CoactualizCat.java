...
/**
 * Desaplica un asiento o un grupo de asientos.
 * Esta clase no lleva control transaccional (excepto los métodos sumarizarCuentas y recalcularSaldos), 
 * debe hacerlo la rutina que la invoque.
...
public class CoactualizCat {
    ...
    /**
     * Aplica o desaplica un asiento o grupo de asientos por rango de fechas.  
     * Si el número de asiento viene vacío se procesa el rango de fechas, caso 
     * contrario solo se procesa el asiento. Si aplica o desaplica solo depende 
     * del operador recibido (+=aplica,-=desaplica).
     * Si la variable de clase mayorizar está en true entonces también realiza 
     ...
     * @return boolean true = Tuvo éxito, false = no lo tuvo.  Si es false
     * debe utilizar getMensaje_err() para obtener un mensaje del error que ocurrió.
     */
    ...
    /**
     * Este método aplica los saldos registrados en las cuentas de movimiento
     * a las cuentas de mayor.
     ...
     * Pero antes de hacerlo pone todos los campos en cero incluyendo el
     * saldo del año anterior y saldos acumulados a la fecha.
    ...
    /**
     * Este método recalcula únicamente las cuentas de movimiento
     * tomando como parámetros las fechas del periodo en proceso.
